...
export class addTokenExpiry1586696429907 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      'ALTER TABLE `user` ADD `emailProofTokenExpiresAt` datetime NULL',
      undefined
    );
    await queryRunner.query(
      'ALTER TABLE `user` ADD `passwordResetTokenExpiresAt` datetime NULL',
      undefined
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      'ALTER TABLE `user` DROP COLUMN `passwordResetTokenExpiresAt`',
      undefined
    );
